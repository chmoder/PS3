...
#include <stdio.h>
#include <spu_mfcio.h>
#include <single_buffer_list_in_out.h>
...
/* here we allocate the data buffer in local memory to hold the input data.
 *
 * This buffer is aligned on 128 byte boundary for efficient DMA transfer
 */
float local_buffer_in[CHUNK_SIZE * NUM_LIST_ELEMENTS] __attribute__ ((aligned (128)));

/* here we allocate the data buffer in local memory to hold the input data.
 *
 * This buffer is aligned on 128 byte boundary for efficient DMA transfer
 */
float local_buffer_out[CHUNK_SIZE * NUM_LIST_ELEMENTS] __attribute__ ((aligned (128)));
         ...
         unsigned long long envp __attribute__ ((unused)))
{
  ...
  for (i = 0; i < num_chunks; i+= NUM_LIST_ELEMENTS)
  {
    ...
    mfc_getl (local_buffer_in, in_addr, dma_list, NUM_LIST_ELEMENTS * sizeof(mfc_list_element_t), tag, 0, 0);
    ...
    * different places in system memory */
    mfc_putl (local_buffer_out, out_addr, dma_list, NUM_LIST_ELEMENTS * sizeof(mfc_list_element_t), tag, 0, 0);
